const SIAL = (() => {
  ...
  const companyTypes = [
    [
      "transportadora",
      "Transportadora",
      "Opera flota, conductores o servicios logisticos.",
    ],
    [
      "productor",
      "Productor",
      "Asocia fincas, grupos o informacion productiva.",
    ],
    [
      "exportador",
      "Exportador",
      "Participa en procesos comerciales y de despacho.",
    ],
  ...
  function applyShell(activeKey) {
    if (window.SIALCore?.initShell) {
      window.SIALCore.initShell({
        area: "gestion",
        module: "empresas",
        view: activeKey || "empresas",
      });
      return;
    }
    ...
    const items = [
      ...
      [
        "tiposEmpresa",
        "../Gestion%20de%20Transporte/gestion-tipos-empresa.html",
        "Tipos de empresas",
      ],
      [
        "empresaTipo",
        "../Gestion%20de%20Transporte/relacion-empresa-tipo.html",
        "Empresa + tipo",
      ],
      ...
      [
        "alertasContactos",
        "gestion-notificaciones-contactos.html",
        "Alertas por contacto",
      ],
  ...
  function initBasicFormValidation(formSelector = "[data-company-form]") {
    ...
    form.addEventListener("submit", (event) => {
      ...
      qsa("[data-nit]", form).forEach((input) => {
        ...
        if (input.value && !/^[0-9]{6,12}-?[0-9]?$/.test(input.value)) {
          setFieldState(
            input,
            note,
            "El NIT debe tener formato numerico valido.",
            "",
          );
          fail = true;
        }
      ...
      if (companyTypeNote) {
        ...
        companyTypeNote.textContent =
          assignments < 1
            ? "Agrega al menos un tipo de empresa antes de guardar."
            : "Tipos de empresa listos para guardar.";
  ...
  function initCompanyTypeAssignment() {
    ...
    const typeLabel = (value) =>
      companyTypes.find(([typeValue]) => typeValue === value)?.[1] || value;
    const selectedTypes = () =>
      qsa("input[type='checkbox']:checked", typeGrid).map(
        (input) => input.value,
      );

    const renderDraft = () => {
      typeGrid.innerHTML = companyTypes
        .map(
    ...
    const renderAssignments = () => {
      ...
      if (assignmentNote)
        assignmentNote.textContent = `${assignments.length} tipo${assignments.length === 1 ? "" : "s"} de empresa asignado${assignments.length === 1 ? "" : "s"}.`;
      qsa("[data-remove-company-type]", assignmentList).forEach((button) => {
        button.addEventListener("click", () => {
          assignments = assignments.filter(
            (value) => value !== button.dataset.removeCompanyType,
          );
          renderAssignments();
        });
      });
    ...
    addButton?.addEventListener("click", () => {
      ...
      if (!selected.length) {
        ...
        if (typeNote)
          typeNote.textContent = "Selecciona al menos un tipo para agregarlo.";
        return;
      }
      assignments = Array.from(new Set([...assignments, ...selected]));
      qsa("input[type='checkbox']", typeGrid).forEach((input) => {
        input.checked = false;
      });
      typeNote?.classList.remove("error");
      typeNote?.classList.add("success");
      if (typeNote)
        typeNote.textContent = "Tipos agregados al resumen inferior.";
      renderAssignments();
    });
      ...
      window.setTimeout(() => {
        ...
        if (typeNote)
          typeNote.textContent =
            "Selecciona uno o mas tipos y agregalos a la asignacion.";
  ...
  function initCompanyRoleTransfer() {
    ...
    qs("#includeRole")?.addEventListener("click", () =>
      move(excluded, included),
    );
    qs("#excludeRole")?.addEventListener("click", () =>
  ...
  function initRoleAdminPanel() {
    ...
    const companyLabel = () =>
      roleCompanyAssignment?.selectedOptions?.[0]?.textContent || "";
    let currentMode = "new";

    const selectedPermissions = () =>
      qsa("[data-permission-check]:checked", form).map((input) => input.value);
      ...
      if (permissionNote) {
        ...
        permissionNote.textContent =
          total === 0
            ...
            : `${total} permiso${total === 1 ? "" : "s"} seleccionado${total === 1 ? "" : "s"}.`;
      }
      if (roleSummaryText) {
        const role = roleName?.value?.trim() || "Nuevo rol";
        const company = roleCompanyAssignment?.value
          ? companyLabel()
          : "empresa pendiente";
    ...
    const openPanel = () => {
      ...
      formTitle.textContent =
        currentMode === "new" ? "Nuevo rol" : "Editar rol";
    ...
    const fillFormFromRow = (row) => {
      ...
      if (roleCompanyAssignment)
        roleCompanyAssignment.value = row.dataset.companyCode || "";
    ...
    const setNewMode = () => {
      ...
      if (permissionNote)
        permissionNote.textContent =
          "Selecciona al menos un permiso para guardar.";
      if (roleSummaryText)
        roleSummaryText.textContent =
          "Nuevo rol para SIAL Central - 0 permisos";
    ...
    const filterRows = () => {
      ...
      empty?.classList.toggle("show", visible === 0);
      if (count)
        count.textContent = `${visible} rol${visible === 1 ? "" : "es"} visibles`;
    };
    ...
    [roleName, roleScope, roleRisk, roleDescription, roleCompanyAssignment]
      .filter(Boolean)
      .forEach((control) => {
  ...
  function initGenericForm(formSelector = "[data-generic-form]") {
    ...
    form.addEventListener("submit", (event) => {
      ...
      qsa("[data-unique-list]", form).forEach((input) => {
        const note = qs(`#${input.getAttribute("aria-describedby")}`);
        const values = input.dataset.uniqueList
          .split("|")
          .map((v) => v.trim().toUpperCase());
        const duplicate =
          input.value && values.includes(input.value.trim().toUpperCase());
        if (duplicate) {
          setFieldState(
            input,
            note,
            "El valor ya existe en la tabla maestra.",
            "",
          );
          fail = true;
        }
      ...
      if (!fail) {
        ...
        ok.innerHTML =
          '<svg class="icon" viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"></path></svg><span>Registro guardado correctamente.</span>';
  ...
  function initContactForm() {
    ...
    const fields = [
      ...
      [
        "companyFarm",
        "Selecciona la empresa o finca asociada.",
        (v) => Boolean(v),
      ],
      [
        "email",
      ...
      [
        "phone",
        "El telefono principal es obligatorio.",
        (v) => v.trim().length >= 7,
      ],
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (!fail) {
        ...
        ok.classList.add("notice-success");
        ok.innerHTML =
          '<svg class="icon" viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"></path></svg><span>Contacto guardado correctamente.</span>';
        window.scrollTo({ top: 0, behavior: "smooth" });
      }
    });
    form.addEventListener("reset", () => {
      setTimeout(() => {
        qsa(".is-error", form).forEach((node) =>
  ...
  return {
    ...
    initCompanyRoleTransfer,
    initCompanyTypeAssignment,
